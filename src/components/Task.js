  ...
  render() {
    return (
      <div className="task" id={`${this.props.task.id}`}>
        ...
        <form>
          <input name="done" type="checkbox" checked={this.props.task.done} onChange={() => this.props.onChangeTaskStatus(this.props.task.id)} />
        </form>

        <button onClick={() => this.props.onRemoveTask(this.props.task.id)} className="trash-button">
